...
import { createDefaultWorkflowsConfig, DEFAULT_ARCHIVE_ROOT, DEFAULT_COMMAND_NAME, DEFAULT_LAUNCHER_AGENT, DEFAULT_LAUNCHER_MODEL, DEFAULT_PARTICIPANTS, DEFAULT_PLANNER_TEMPLATE, DEFAULT_REVIEWER_TEMPLATE, } from "./defaults.js";
...
function relativeOrAbsolute(file, root) {
...
function normalizeParticipants(input) {
    ...
    if (!Array.isArray(input))
        throw new Error("Plan Arena option `participants` must be an array");
    return input.map((item, index) => {
        ...
        if (!id)
            throw new Error(`Plan Arena participant at index ${index} must define id`);
        ...
        const label = typeof record.label === "string" && record.label.trim()
            ...
            : model
                ? model
                : id;
        const agent = typeof record.agent === "string" && record.agent.trim() ? record.agent.trim() : `plan-arena-${slug(id)}`;
        return {
            id,
            label,
            agent,
...
function applyOptionOverrides(config, options) {
    if (options.archiveRoot)
        config.archiveRoot = options.archiveRoot;
    if (options.defaultWorkflow)
        config.defaultWorkflow = options.defaultWorkflow;
    for (const workflow of Object.values(config.workflows)) {
        if (options.archiveRoot)
            workflow.archiveRoot = options.archiveRoot;
        if (options.participants)
            workflow.participants = options.participants;
        if (options.language)
            workflow.language = options.language;
        if (options.concurrency !== undefined)
            workflow.concurrency = Math.max(1, Math.floor(options.concurrency));
        if (options.retry !== undefined)
            workflow.retry = Math.max(0, Math.floor(options.retry));
    }
    return config;
}
async function loadWorkflowConfig(projectRoot, options) {
    if (options.workflowPath) {
        const workflowPath = resolveUserPath(options.workflowPath, projectRoot);
        const config = await readJson(workflowPath);
        return {
...
async function loadInstructions(projectRoot, files = []) {
    const chunks = [];
...
async function loadTemplate(configuredPath, configDir, projectRoot, fallback) {
    if (!configuredPath)
        return fallback;
...
function assertHeadings(output, headings, taskId) {
...
async function runQueue(items, concurrency, run) {
    const queue = [...items];
...
function collectParticipants(config) {
...
function participantAgent(participant) {
    return {
        ...
        prompt: "You are a Plan Arena participant. Follow the supplied task template exactly. Produce planning or review text only. Do not modify files, run shell commands, search the web, or call subagents.",
        options: {},
    };
}
function launcherAgent(model) {
    return {
...
async function injectOpenCodeConfig(cfg, projectRoot, options) {
    ...
    config.command[options.commandName] = {
        ...
        template: "Call the `plan_arena_run` tool exactly once. Pass the raw command arguments below in the `arguments` field without rewriting them. Do not inspect the repository yourself.\n\n$ARGUMENTS",
    };
    config.agent ??= {};
...
export const PlanArenaPlugin = async ({ client, directory, worktree }, pluginOptions) => {
        ...
        tool: {
            plan_arena_run: tool({
                ...
                async execute(args, context) {
                    ...
                    const plannerTemplate = await loadTemplate(workflow.templates?.planner, loaded.configDir, projectRoot, DEFAULT_PLANNER_TEMPLATE);
                    const reviewerTemplate = await loadTemplate(workflow.templates?.reviewer, loaded.configDir, projectRoot, DEFAULT_REVIEWER_TEMPLATE);
                    ...
                    const runModelTask = async (task, prompt, requiredHeadings) => {
                        if (shouldSkip(task)) {
                            await saveManifest(manifest);
                            return;
                        }
                        for (let attempt = 1; attempt <= retry + 1; attempt++) {
                            ...
                            try {
                                ...
                                if (!task.outputFile)
                                    throw new Error(`Task ${task.id} has no outputFile`);
